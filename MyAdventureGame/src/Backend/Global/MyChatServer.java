...
public class MyChatServer
	{
		...
		/**
		 * The set of all the print writers for all the clients. This set is
		 * kept so we can easily broadcast messages.
		 */
...
//				}
		 ...
		 * and are responsible for a dealing with a single client and
		 * broadcasting its messages.
		 */
		private static class Handler implements Runnable
			{
				private String name = AdventureMain._adventure._mpPanel.nick.getText();
				private Socket socket;
				private BufferedReader in;
				private PrintWriter out;

				/**
				 * Constructs a handler thread, squirreling away the socket. All
				 * the interesting work is done in the run method.
				 */
				public Handler(Socket socket)
					{
						this.socket = socket;
						System.out.println("angekommen8");
				 ...
				 * acknowledges the name and registers the output stream for the
				 * client in a global set, then repeatedly gets inputs and
				 * broadcasts them.
				 */
				public void run()
					{
						try
						{
							System.out.println("angekommen7");

							// Create character streams for the socket.
							in = new BufferedReader(new InputStreamReader(socket.getInputStream()));
							out = new PrintWriter(socket.getOutputStream(), true);

							// Request a name from this client. Keep requesting
							// until
							// a name is submitted that is not already used.
							// Note that
							// checking for the existence of a name and adding
							// the name
							...
							// Now that a successful name has been chosen, add
							// the
							// socket's print writer to the set of all writers
							// so
							// this client can receive broadcast messages.
							out.println("NAMEACCEPTED");
							writers.add(out);

							// Accept messages from this client and broadcast
							// them.
							// Ignore other clients that cannot be broadcasted
							// to.
							while (true)
							{
						...
						{
							// This client is going down! Remove its name and
							// its print
